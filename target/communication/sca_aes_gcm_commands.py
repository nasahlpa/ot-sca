...
class OTAESGCM:
    ...
    def fvsr_batch(self, num_batches: int, triggers: list[bool], trigger_block: int,
                  iv: list[int], iv_len_bytes: int, key: list[int],
                  key_len_bytes: int, num_aad_blocks: int, num_ptx_blocks: int,
                  aad, last_aad_len, ptx, last_ptx_len):
        ...
        Args:
            ...
            key: Fixed key array.
            key_len_bytes: Number of bytes for the key.
        ...
        if self.simple_serial:
            ...
            time.sleep(0.01)
            # Transmit IV.
            data = {"block": iv, "num_valid_bytes": iv_len_bytes}
            self.target.write(json.dumps(data).encode("ascii"))
            time.sleep(0.01)
            # Transmit key.
            data = {"key": key, "key_length": key_len_bytes}
            self.target.write(json.dumps(data).encode("ascii"))
            time.sleep(0.01)
